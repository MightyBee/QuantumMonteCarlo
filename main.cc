		...
		for(size_t j(0); j < N_part; j++){
			for(size_t k(0); k < N_slices; k++){
				...
				s_old = 0.5*m*pow((system[nn][mm_plu]-system[nn][mm])/d_tau,2)
						...
						//+0.5*m*omega*omega/d_tau/d_tau*pow(system[nn][mm],2); // harmonic potential
				...
				s_new = 0.5*m*pow((system[nn][mm_plu]-new_r)/d_tau,2)
						...
						//+0.5*m*omega*omega/d_tau/d_tau*pow(new_r,2);
...
double bissectionAction(const double& m, const double& omega, const double& d_tau, const size_t& start, const vector<double>& path, const double& displacement){
	double ba(0.0);
	size_t s(path.size());
	for(size_t i(0); i<s/3; i++){
		ba+=//0.5*pow(path[(i+start)%s]+displacement,2);
			+0.1*m*omega*omega*pow(path[(i+start)%s]+displacement,4)
			-5.0*m*omega*omega*pow(path[(i+start)%s]+displacement,2);
	}
	ba+=0.5*m*pow(((path[start]+displacement)-path[(start+s-1)%s])/d_tau,2);
	ba+=0.5*m*pow((path[(start+s/3)%s]-(path[(start+s/3-1)%s]+displacement))/d_tau,2);
	return ba;
}
